...
double Random :: Exp(double param) {
   // This function generates a random number from an esponential distribution with given parameter (lambda)
   return -log(1.-Rannyu())/param;
}

double Random :: Lorentz(double mean, double HWHM) {
   // This function generates a random number from a Cauchy-Lorentz distribution with given mean and HWHM
   return mean + HWHM*tan(M_PI*(Rannyu()-0.5));
}

int Random :: Integer(int min, int max){
   // This function generates a random integer between min and max included
   return min+(max-min+1)*Rannyu();
}
