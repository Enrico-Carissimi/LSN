...
/* template <>
void Population<Position2D>::setIndividualScore(int j){
    double sum = 0.;
    ...
    for (int i = 0; i < individualSize; i++){
        sum += distance2(positions[v[i] - 1], positions[v[i + 1] - 1]);
    }
...
template <>
void Population<double>::setIndividualScore(int j){
    double sum = 0.;
    ...
    for (int i = 0; i < individualSize; i++){
        // radius is set to 1, positions can be scaled when drawn if the actual circle has a different radius
        sum += distanceOnCircle(1., positions[v[i] - 1], positions[v[i + 1] - 1]);
    }
