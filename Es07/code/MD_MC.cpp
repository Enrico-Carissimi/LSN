...
{ 
  if (Start(argc, argv) == -1) return -1;
  Input(); //Inizialization
  int nconf = 1;

  //Autocorr.open("data-7.2.out",ios::app);
...
{
  if (argc != 2){
    cout << "Program usage: " << argv[0] << " <x>" << endl;
    cout << "Where <x> is the state to simulate:\n\ts for solid\n\tl for liquid\n\tg for gas\n\ti to start from current input.in file" << endl << endl;
    return -1;
  }
  state = argv[1];
  if (state == "s"){state = "solid/"; return 0;}
  if (state == "l"){state = "liquid/"; return 0;}
  if (state == "g"){state = "gas/"; return 0;}
  if (state == "i"){state = ""; return 0;}

  cout << "Only parameters \"s\", \"l\", \"g\" or \"i\" are accepted" << endl;
  return -1;
}
...
//Read input informations
  ReadInput.open(state + "input.in");

  ReadInput >> iNVET;
  ReadInput >> restart;

  state = (iNVET) ? state+"MC/" : state+"MD/"; //thermalization is different for MD and MC simulations

  if(restart) Seed.open(state + "seed.out");
...
//Prepare variables for the histogram of g(r)
  nbins = 100;
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  iw = 4; //Pressure
  ig = 5; //Start of histogram
  n_props = 5+(int)nbins; //Number of observables

//Read initial configuration
  cout << "Read initial configuration" << endl << endl;
  if(restart)
  {
    ReadConf.open(state + "config.out");
    ReadVelocity.open(state + "velocity.out");
...
//Tail corrections
  vtail = 8.0/3.0*M_PI * rho * (1.0/(3.0 * pow(rcut,9)) - 1.0/pow(rcut,3));
...
//reset histogram
  for(int i=0; i<nbins; ++i) walker[ig+i] = 0.0;
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        vij = 1.0/pow(dr,12) - 1.0/pow(dr,6); // 1/r^12 - 1/r^6
        v += vij;
        wij = vij + 0.5/pow(dr,6); // 1/r^12 + 1/2 1/r^6
        w += wij;
      }

//update the histogram of g(r)
      walker[ig + (int)floor(dr/bin_size)] += 2; // walker[ig] to walker[ig+nbins-1] is our histogram of g(r)
    }          
  }
  ...
  walker[iw] = rho * walker[it] + 16.0 / vol * w;

  //Autocorr << walker[iv]/(double)npart + vtail << endl; //write es 7.2 data
...
{
   ofstream Epot, Ekin, Etot, Temp, Pres, Gr, GrFin;
   const int wd=12;
    ...
    Gr.open("output_gr.dat",ios::app);
    GrFin.open("output_grfinal.dat",ios::app);
...
//Total energy
    Etot << setw(wd) << iblk <<  setw(wd) << setprecision(7) << stima_etot << setw(wd) << setprecision(7) << glob_av[ie]/(double)iblk << setw(wd) << setprecision(6) << err_etot << endl;
//Temperature
...
//g(r)
    ...
    {
      double norm = 4./3.*pi*rho*npart * (pow((i+1)*bin_size, 3) - pow(i*bin_size, 3));
      stima_gdir = blk_av[ig+i]/norm/blk_norm;
      glob_av[ig+i] += stima_gdir;
      glob_av2[ig+i] += stima_gdir*stima_gdir;
      
      Gr << setw(wd) << glob_av[ig+i]/iblk;
      if(iblk==nblk)
      {
        err_gdir=Error(glob_av[ig+i],glob_av2[ig+i],iblk);
        GrFin << setw(wd) << i*bin_size <<  setw(wd) << glob_av[ig+i]/iblk << setw(wd) << err_gdir << endl;
      }
    }
    Gr << endl;
